...
#MULTINOMIAL STEP
mod2=step(mod1)
prob <- predict(mod2, test, type="prob")      
predstep <- ifelse(prob > 0.45, 1, 0)
eval(test$y, as.numeric(predstep))
...
pred2=knn(trainknn[,-23], testknn[,-23],trainknn$y, k=7)
eval(test$y, as.numeric(pred2))
...
mod4=rpart(y~., method="class", data=train, minsplit=560, minbucket=235, cp=0.0005, maxdepth=11)
...
#Boosting
mod7 = boosting(y~., train, boos = TRUE, mfinal = 10)
...
curvaROC=function(pred,y,grafico=F){
  ...
  if(grafico){
    ...
    p<-plot_ly(x=FP, y=FP, name = 'Linea no Discrimina',
...
for(i in 1:10){
  train=datos_finales[-cortes[[i]],]
  test=datos_finales[cortes[[i]],]
  mod=rpart(y~., method="class", data=train, minsplit=560, minbucket=235, cp=0.0005, maxdepth=11)
