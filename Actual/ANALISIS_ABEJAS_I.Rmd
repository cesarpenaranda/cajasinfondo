...
```
**Se oberseva una tendencia al aumento pero minima**

- Grafico pendientes por zona
```{r}
xyplot(conteo~mes,group=zona,type=c("r","p"),data=base_abejas,auto.key=list(columns=5))
```
**Hay una tendencia al aumento minima, en finca garzas mas bien hay una tendencia negativa**
- Grafico pendiente por especie segun zona
```{r}
xyplot(conteo~mes|zona,group=especie,type=c("r","p"),data=base_abejas,auto.key=list(columns=5))
...
```{r}
mod.p1=glmer(conteo~especie*mes+(1|zona)+(mes|zona),family = poisson,data = base_abejas)
summary(mod.p1)
```

- Modelo aleatorio binomial negativa
```{r}
mod.b1=glmer.nb(conteo~especie*mes+(1|zona)+(mes|zona),data = base_abejas)
summary(mod.b1)
```
...
H0 pendientes aleatorias iguales
```{r}
#mod.b1=glmer.nb(conteo~especie*mes+(1|zona)+(0+mes|zona),data = base_abejas)
