...
```{r}
#estos son los promedios observados o obtenidos al realizar el experimento
(med=tapply(base$proteina,list(base$cond,base$genero),mean))
```

#(e) Promedios estimados bajo el modelo sin interacción:
```{r}
#recordemos que estas son estimaciones bajo el modelo sin interaccion donde m11=mu+a1*alfa+b1*beta

m11=medgen+alfa[1]+beta[1] #macho;alimento=43.11875
m21=medgen+alfa[2]+beta[1] #hembra;alimento=39.28125
m12=medgen+alfa[1]+beta[2] #macho;dieta=40.00625
m22=medgen+alfa[2]+beta[2] #hembra;dieta=36.16875
...
```{r}
library(ggplot2) #recordar que en group se pone el factor de mayor interes en el estudio, o factor de diseño en este caso el mayo interes se encontraba en la condicion de alimentacion
...
#(i) Efectos simples:
```{r}
#esta es la forma de conseguir los efectos(alfa,beta) con el modelo
...
```{r}
(v1=mean(v)) #esto es porque el experimento es balanceado
...
#(e) Conclusión:
```{r}
(f=0.18/7.54) #f=0.02387268
pf(f,1,12,lower.tail = F) #1 gl del CMinteraccion y 12 delgl de CMres 
```
...
#      39.64        1.56        1.92
#en este caso no olvidemos que los faltantes cond2 y genero2 son iguales pero con signo negatico en este caso
```
...
table(base$cond)
table(base$genero)
c(sum(8*alfa^2)/1, sum(8*beta^2)/1) #8 son las replicas por tratamiento 1 son los gl correspondientes
#CMtratcond 
#CMtratgenero
```
...
```{r}
(efint=med-mest) #estos son mis efectos de interaccion
```
...
##(b) Matriz de estructura:
```{r}
contrasts(base$cond)#me indica las auxiliares
...
#     36.000         -3.375          2.375          1.000         -0.875          2.625 
```
...
##(a) Gráfico de líneas:
Realice la visualización usando base1. Haga un gráfico de líneas para ver el
...
```{r}
library(ggplot2)#bloque es la persona osease el bloque
...
##(c)¿Cuál es la forma correcta de obtener la variancia residual?  Variancia residual correcta:
...
##(a) Visualización: Realice la visualización usando los datos con tres tipos de agua (base2)
```{r}
#Recordar
#bloque es persona ; el tiempo la respuesta ; agua es un factor (destilada,grifo,añejada)
ggplot(base2, aes(x=bloque, y=tiempo, group = agua)) +
stat_summary(fun="mean", geom="line", aes(linetype = agua))+labs(x = "persona")
```
...
```
        a-d         a-n         n-d 
0.007220039 0.149694640 0.195679557 
...
res1=base$res-predict(mod1)+mean(base$res) #variable centrada
...
sum(anova(mod2)[,2]) #esta es la suma de toda la columa Sum sq del anova
```

```{r}
n=nrow(base);n #total de dias o bloques = 3*7
(sctot=(n-1)*var(base$res)) #esta es la formula a pie para obtener la SCtotal = 2600.286
```

##(b) Suma de cuadrados residual (SCRes):
```{r}
(scres=anova(mod2)[3,2]) #168.5714
```

##(c) Suma de cuadrados de bloque (SCB):
```{r}
table(base$dia) #me dice cuantos trats se aplicaron por dia en este caso fueron 3 por cada dia de los 7 dias, p=3
```

```{r}
p=3 
m.b=tapply(base$res,base$dia,mean) #mu de la respuesta segun dia
mgen=mean(base$res) #media general 
(scb=sum(p*(m.b-mgen)^2)) #formula a pie para obtener la SCbloque 1114.286
```
...
```{r}
a=7; p=3; r=3 #r=replicas #a=numero de tratamientos
lambda=r*(p-1)/(a-1)
(sctrat=p*sum(Qi^2)/(lambda*a)) #Qi son los ajustados La SCTrat.aj es 1317.43.
```
La SCTrat.aj es 1317.43.

##(c) Suma de cuadrados residual:
```{r}
scres2=sctot-scb-sctrat;scres2 #168.5714
```
#5. Prueba de hipótesis de igualdad de medias:
##(a) Conclusión:
```{r}
#bloques=7 ; n=21 ; a=7 trats 
cmtrat=sctrat/(a-1) #a es el numero de trats
cmres=scres2/8 #8 grados de libertad residuales ;n-a-b+1=21-7-7+1=8
(f=cmtrat/cmres) #obteniendo el f correspondiente
```

```{r}
pf(f,6,8,lower.tail = F) #probabilidad asociada al f obtenido
#6=a-1 =num
#8=n-a-b+1 =den0
```
