...
###Funcion para grafico de valores extremos
graf=function(x){
  dffits=dffits(x)
  estand=rstandard(x)
  plot(x$fitted,estand, cex=0.2*abs(dffits)/mean(abs(dffits)),xlim=c(-50000,250000),ylim=c(-3,8))
  abline(h=0, col=6)
  title("Mod.grande: Residuos por valores predichos, ponderados por DFFIT")
  text(x$fitted[estand>4],estand[estand>4],identif[estand>4])
}
graf(mod.grande)
graf(mod.reducido)
...
plot(gasto_ocio~as.factor(sexojefe))
...
"logaritmo es una transformacion, muy utilizada
porque nos permite hacer interpretaciones"
...
"Por cada año adicional de escolaridad del jefe
el gasto en ocio me aumentaria en 3.8%"

# lnpropalim     -2.947e-01
"por cada 1% de aumento en la proporcion en gasto en alimentos
el gasto en osio se reduce en 0.29%
"
...
crPlots(mod.grande3)
"Linea azul y rozada ahi me doy cuenta si hay linealidad 
si veo que se alejan es porque necesita trasformarse"
...
#box cox muchas veces me mejora el ajuste osea el rcuadrado ajustado
###Transformacion Box-Cox


boxcox(gasto_ocio~tamhogar+sexojefe+edadjefe+escoljefe+miembrosocup+
       miembrospercep+ingresototal+propalim+alquilada+prestamcasa+numvehic+
       pobmenores)

bcobjeto=boxcox(gasto_ocio~tamhogar+sexojefe+edadjefe+escoljefe+miembrosocup+
...
"El problema de estas trasformaciones a raiz decima y octava
es que no podemos interpretar los betas directamente"
